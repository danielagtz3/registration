...
class Graph:
    ...
    def has_cycle(self):
        ...
        def dfs(vertex, path_stack):
            index = self.get_index(vertex)
            if index in path_stack:
                return True
            path_stack.add(index)
            for neighbor in self.get_adjacent_vertices(index):
                if dfs(self.vertices[neighbor].label, path_stack):
                    return True
            path_stack.remove(index)
            return False

        for vertex in self.vertices:
            if dfs(vertex.label, set()):
                return True
        return False
    ...
    def get_registration_plan(self):
        ...
        while temp_vertices:
            ...
            # Add up to 4 courses per semester
            semester = semester[:4]
            result.append(semester)

            # Remove courses from the graph and update in-degree
            for course in semester:
                delete_vertex_from_copy(course, temp_matrix, temp_vertices)
                in_degree = [sum(row) for row in zip(*temp_matrix)]
...
def main():
    ...
    for _ in range(num_edges):
        start_label, finish_label = input().strip().split()
        start = graph.get_index(start_label)
        finish = graph.get_index(finish_label)
